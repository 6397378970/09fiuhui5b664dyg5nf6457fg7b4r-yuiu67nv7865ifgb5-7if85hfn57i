...
async def cmd_start(message: types.Message):
    ...
    text = (
        ...
        f"{AUTHORIZED_EMOJI} {'Authorized' if is_auth else 'Not Authorized'}"
    )
...
async def cmd_spam(message: types.Message):
    ...
    print(f"✅ /spam from {message.from_user.id}")
    ...
    if len(args) < 2:
        await message.reply("⚠️ Usage: `/spam message`", parse_mode=ParseMode.MARKDOWN)
        return
...
async def cmd_stopspam(message: types.Message):
    ...
    print(f"✅ /stopspam from {message.from_user.id}")
...
async def cmd_status(message: types.Message):
    print(f"✅ /status from {message.from_user.id}")
    ...
    if spam_active:
        ...
        await message.reply(f"{NO_SPAM_EMOJI} *NO SPAM*\n🤖 Total Bots: {TOTAL_BOTS}\n✅ Approved Users: {len(approved_users)}", parse_mode=ParseMode.MARKDOWN)

@dp.message(Command("approve"))
async def cmd_approve(message: types.Message):
    print(f"✅ /approve from {message.from_user.id}")
    
    if message.from_user.id != OWNER_ID:
        await message.reply("❌ Only owner can approve!")
        return
...
async def cmd_dapprove(message: types.Message):
    print(f"✅ /dapprove from {message.from_user.id}")
    
    if message.from_user.id != OWNER_ID:
        await message.reply("❌ Only owner can dapprove!")
        return
    ...
    if clean_target not in approved_users:
        await message.reply(f"❌ {target} not approved!")
        return
...
async def cmd_approvedlist(message: types.Message):
    print(f"✅ /approved from {message.from_user.id}")
    
    if message.from_user.id != OWNER_ID:
        await message.reply("❌ Only owner can view!")
        return
